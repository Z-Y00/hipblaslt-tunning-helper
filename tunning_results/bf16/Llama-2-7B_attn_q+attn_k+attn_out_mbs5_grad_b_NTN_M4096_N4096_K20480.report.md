# Llama-2-7B — attn_q+attn_k+attn_out [grad_b]

| Parameter | Value |
|-----------|-------|
| Model | Llama-2-7B |
| Layer | attn_q+attn_k+attn_out |
| Phase | grad_b |
| MBS | 5 |
| Trans (ABC) | NTN |
| M | 4096 |
...
|--------|-------:|----------:|---------:|-------:|
| **Tensile tuned** | 1424.63 | 482.37 | 113.4% | 100.6% |
| **API bench (installed)** | 1415.58 | 485.45 | 112.7% | — |
| **hipblaslt-bench (stock)** | 1255.79 | 547.22 | — | — |

> **Tuned/API ratio (gate metric):** 100.64%
...
```bash
/opt/rocm/bin/hipblaslt-bench -m 4096 -n 4096 -k 20480 --precision bf16_r --compute_type f32_r --transA N --transB T -i 50 -j 30 --rotating 1920 --print_kernel_info
```
...
Rotating buffer 1920 MiB. Needed Size: 352 MiB. Needed block count: 6 (Capped to max iters: 50)
...
[0]:transA,transB,grouped_gemm,batch_count,m,n,k,alpha,lda,stride_a,beta,ldb,stride_b,ldc,stride_c,ldd,stride_d,a_type,b_type,c_type,d_type,compute_type,scaleA,scaleB,scaleC,scaleD,amaxD,swizzle_a,swizzle_b,activation_type,bias_vector,bias_type,aux_type,rotating_buffer,flush,use_gpu_timer,hipblaslt-Gflops,hipblaslt-GB/s,us
    N,T,0,1,4096,4096,20480,1,4096,83886080,0,4096,83886080,4096,16777216,4096,16777216,bf16_r,bf16_r,bf16_r,bf16_r,f32_r,0,0,0,0,0,0,0,none,0,bf16_r,bf16_r,1920,0,0,1.25579e+06,628.175,547.22
